// student-app/src/screens/main/ClassesScreen.tsx
...
import { Session } from "../../types/student.types";
...
export const ClassesScreen: React.FC = () => {
  ...
  const { markAttendanceWithBiometric, checkDeviceRegistration, isRegistered } =
    useBiometric();
  ...
  const validateSession = async (
    ...
    try {
      ...
      return { valid: result.canMark, message: result.message };
    } catch (error: any) {
      return {
        valid: false,
  ...
  const handleMarkAttendance = async (session: Session) => {
    ...
    // Check 1: Validate session with backend
    const { valid, message } = await validateSession(session);
    if (!valid) {
      Alert.alert("Cannot Mark Attendance", message);
    ...
    Alert.alert(
      ...
      [
        ...
        { text: "Use QR Code", onPress: () => setMarkingMethod("qr") },
      ],
    );
  };
  ...
  const markWithBiometric = async (session: Session) => {
    ...
    if (!registered) {
      Alert.alert(
        "Biometric Not Setup",
        "Please setup biometric authentication first in Profile",
        [{ text: "OK", onPress: () => setSelectedSession(null) }],
      );
      return;
    }
    ...
    try {
      ...
      if (!isOnline) {
        ...
        await addToQueue({
          ...
          deviceId: (await checkDeviceRegistration()) ? "device_id" : "unknown",
          location,
        });
        Alert.alert(
          "Saved Offline",
          "Attendance will be synced when you regain internet connection",
        );
      } else {
        // Online mode - mark immediately
        const result = await markAttendanceWithBiometric(session._id, location);
        if (result.success) {
          Alert.alert("Success", "Attendance marked successfully!");
          loadSessions(); // Refresh to update marked status
        } else {
    ...
    try {
      ...
      if (!isOnline) {
        await addToQueue({
          ...
          timestamp: new Date().toISOString(),
          deviceId: "qr_fallback",
          location,
        });
        Alert.alert(
          "Saved Offline",
          "QR attendance will be synced when online",
        );
      } else {
    ...
    >
      ...
      {item.sessionCode && (
        <View style={styles.codeContainer}>
          ...
          <Text style={styles.codeValue}>{item.sessionCode}</Text>
        </View>
      )}
  ...
  return (
    <View style={styles.container}>
            ...
            {activeSessions.length === 0 && (
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptyText}>
                  There are no live sessions right now. Check back during your
                  class time.
                </Text>
              </View>
            )}
      ...
      {/* QR Modal */}
      <Modal visible={markingMethod === "qr"} transparent animationType="slide">
